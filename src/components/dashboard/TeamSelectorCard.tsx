...
const TeamSelectorCard = () => {
  ...
  return (
    <div className="relative">
      <div className="w-fit bg-clr-850 p-2 rounded-lg text-clr-100 flex items-center justify-center gap-x-2 cursor-pointer" onClick={() => setShowTeamMembers(prevState => !prevState)}><div style={{ backgroundColor: team?.badgeColor }} className="w-2 h-2 rounded-full"></div>{team?.name}<img src={Caret} className="w-8 h-8" /></div>
